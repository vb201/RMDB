...
const apiSettings = {
  ...
  // Fix
  fetchAll: async (searchTerm: string, page: number): Promise<TV> => {
    const endpoint: string = searchTerm
      ? `${SEARCH_MULTI_BASE_URL}${searchTerm}&page=${page}`
      : `${POPULAR_TV_BASE_URL}&page=${page}`;
    return await (await fetch(endpoint)).json();
  },
  fetchMovieVideo: async (movieId: string): Promise<MovieVideo> => {
    const endpoint: string = `${API_URL}movie/${movieId}/videos?api_key=${API_KEY}&language=en-US`;
    return await (await fetch(endpoint)).json();
  },
